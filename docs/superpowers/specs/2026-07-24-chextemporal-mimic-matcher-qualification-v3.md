# CheXTemporal-MIMIC Matcher Qualification v3
...
## Direct v2 base and terminal evidence

The direct base is the frozen v2 protocol snapshot at SHA-256
`e572c8fed8e94f29cff53ee1bcd11474c67a73e4671c352be0a98808fcd01b13`.
Its one no-retry process-A run is immutable scientific-gate evidence:
...
- status `FAIL_Q2_FEATURE_INTEGRITY`;
- repeat maximum absolute difference `3.814697265625e-05`;
- Q0, Q1, Q3, Q4, and Q5 passed; B was not started.

The v2 repeat compared the first eight examples extracted inside a batch of 64
against a fresh batch of eight. Different batch shapes can select different
floating kernels and are not the same registered operation. v3 retains the
strict exact-zero requirement and changes only the repeat sample size to the
registered extraction batch size, so both forward passes use the same ordered
inputs and shape. No tolerance, metric threshold, crop, cohort, encoder,
matcher, seed, or bootstrap setting changes.
...
- qualification runner SHA-256:
  `ba909b327b032ce7f4888cdaebb61f603036241c2d71bc06b354e59636d02a4a`;
...
- focused test hashes:
  ...
  `bc657394ae092567c5006391b0228f601b9577cdacbd6ef7241386bebda5edff`,
  and
  `cb1369b8fc84bcf4dacae693937f7bb147023cf76051a42c7d9c20d750179915`.

The frozen focused suite passes `16/16`; Ruff, format, and py_compile checks
pass. Source changes require a new protocol version and output namespace.

The pre-freeze GPU1 repeat-shape diagnostic re-extracted all 680 crops using
the registered batch size 64. It produced exact
`repeat_max_abs_difference = 0.0` and feature-ledger SHA-256
