...
def aggregate(config_path: Path) -> dict[str, Any]:
    ...
    if output.exists():
        raise FileExistsError("R48 confirmation aggregate must be fresh")
    ...
    if (
        ...
        raise PermissionError("R48 confirmation baseline receipt drift")
    ...
    checks = (
        ("true_macro_f1", metrics["true_pair"]["macro_f1"],
         gate["true_macro_f1_at_least"], "at_least"),
        ("true_minus_prior_shuffle",
         comparisons["true_vs_prior_shuffle"]["effect_pp"],
         gate["true_minus_prior_shuffle_at_least_pp"], "at_least"),
        ("true_minus_prior_shuffle_ci95_lower",
         comparisons["true_vs_prior_shuffle"]["ci95_lower_pp"],
         gate["true_minus_prior_shuffle_ci95_lower_above_pp"], "above"),
        ("true_minus_current_only",
         comparisons["true_vs_current_only"]["effect_pp"],
         gate["true_minus_current_only_at_least_pp"], "at_least"),
        ("true_minus_current_only_ci95_lower",
         comparisons["true_vs_current_only"]["ci95_lower_pp"],
         gate["true_minus_current_only_ci95_lower_above_pp"], "above"),
        ("true_minus_query_only",
         100.0 * (metrics["true_pair"]["macro_f1"]
                  - metrics["query_only"]["macro_f1"]),
         gate["true_minus_query_only_at_least_pp"], "at_least"),
    )
    for name, observed_value, required_value, direction in checks:
        observed = float(observed_value)
        required = float(required_value)
        failed = observed < required if direction == "at_least" else observed <= required
        if failed:
            failures.append({
                "gate": name,
                "observed": observed,
    ...
    for label, observed_value in metrics["true_pair"]["per_class_recall"].items():
        observed = float(observed_value)
        if observed < floor:
            failures.append({
    ...
    for metric_name in ("schema_validity", "finding_echo_accuracy"):
        observed = float(metrics["true_pair"][metric_name])
        required = float(gate[metric_name])
    ...
    passed = not failures
    result = {
        "schema": "visualvit.prta-gen.r48-fprr-confirmation-aggregate.v1",
        "status": (
            config["result_statuses"]["confirmation_go"]
            if passed
            else config["result_statuses"]["confirmation_stop"]
        ),
        ...
        "internal_replication_claim_allowed": passed,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate R48 confirmation")
