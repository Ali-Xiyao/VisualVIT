...
from scripts import run_r28b_calibrated_choice_tier as r28b


PROCESS_A_DEFAULT = Path(
    r"F:\VisualVIT_runtime\050_routeC\r28_case_study_tier"
    r"\tier_r28b_v1_source_closed"
)
PROCESS_B_DEFAULT = Path(
    r"F:\VisualVIT_runtime\050_routeC\r28_case_study_tier"
    r"\tier_r28b_v1_source_closed_repro"
)
CERTIFICATE_DEFAULT = Path(
    r"F:\VisualVIT_runtime\050_routeC\r28_case_study_tier"
    r"\tier_r28b_v1_source_closed_reproduction_certificate.json"
)
DETERMINISTIC_FILES = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Verify fresh-process R28b reproduction"
    )
...
def verify(process_a: Path, process_b: Path) -> dict[str, Any]:
    ...
    checks["attempts_equal"] = (
        manifests["a"]["attempts"]
        == manifests["b"]["attempts"]
        == [r28b.SYSTEM_B1, r28b.SYSTEM_B2]
    )
    failed = sorted(name for name, value in checks.items() if not value)
    return {
        "status": (
            "PASS_R28B_FRESH_PROCESS_REPRODUCTION"
            if not failed
            else "FAIL_R28B_REPRODUCTION"
        ),
