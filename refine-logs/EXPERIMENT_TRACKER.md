...
| S078 | POST_FAILURE_DIAGNOSTIC | fixed-decoder/competence/bypass ladder | STOP_D2_INCOMPETENT | D1 exact reproduction PASS; D2 B4b train F1 `0.733/0.467/1.000`, so D3 is forbidden by the frozen every-seed `>=0.80` gate |
| S079 | QUERY_ANCHOR_ENGINEERING | query-conditioned structural and end-to-end anchor | BLOCKED_IMPLEMENTATION | protocol red-team PASS, but code red-team found no production-path runner, incomplete visible-feature marginal control, and incomplete exact-64 integration |
| D010 | DATA_QUALIFICATION | official asset/license manifest | HOLD_SCHEMA | pinned public annotations downloaded and SHA-verified; 548/1,787 rows have conflicting full prediction keys and released boxes lack per-box progression mapping |
...
`GO_CAPES_CI_V1_S010_S070_SURVIVAL + FAIL_S075_MECHANISM_GATE + STOP_S078_D2_INCOMPETENT + HOLD_D010_SCHEMA + NO_GO_REAL_PILOT_OR_FORMAL_SCALING`
